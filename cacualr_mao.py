...
#sequencia
    ...
    for i in range(x):
        valor_novo.append(1)
    valor_novo = sorted(valor_mao)
    for cont in range(3 + x):
        for i in range(1,15):
...
#royal flush 
                mesa_nova = []
                for num in range(lista[0], lista[4]+1):
                    mesa_nova.append()
                if flush(mesa, lista, 5):    
                    pontos[0] = 9

    return(pontos)
  

valor = calcular_mao(["1e", "8e"], ["4e","3e","2e","5e","7o"])
print(valor)